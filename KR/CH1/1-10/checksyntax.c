...
void validatec(char file[], int len);
...
void validatec(char file[], int len) 
{	
	/* iterate though file and evaluate each type of 
	 * syntax issue
	 * 1) unbalance parenthesis
	 * 2) unbalanced braces
	 * 3) unbalanced brackets
	 * 
	 * ** handle 'in-string' exception */
	int c;
	int line = 1;
	int parenthesis = 0;
	int brace = 0;
	...
	int literal = 0;
	for (c = 0; c <= len; ++c) {
		/* check to see if start of string constant */
		if (file[c] == '\"')
			literal = !literal;		
		if (literal == 0) {				
			if (file[c] == '\n')
			...
			if (file[c] == ')' && parenthesis == 0) {
				printf("unbalanced parenthesis at:\n line: %d\n character number: %d\n", line, c);
				break;
			}		
			if (file[c] == '(') {
				parenthesis = 1;
			...
			if (parenthesis == 1 && (file[c] == '{' || file[c] == '\n')) {
				printf("unbalanced parenthesis at:\n line: %d\n character number: %d\n", line, c);			
				break;
			}
			/* done handling unbalanced parenthesis */
			...
			/* done handling unbalanced brackets*/
			
			/* handle unbalanced braces */
			if (file[c] == ']' && brace == 0) {
				printf("unbalanced brace at:\n line: %d\n character number: %d\n", line, c);
				break;
			}		
			if (file[c] == '[') {
				brace = 1;
			...
			if (brace == 1 && (file[c] == '=' || file[c] == ';' || file[c] == ')' || file[c] == '{' || file[c] == '\n')) {
				printf("unbalanced brace at:\n line: %d\n character number: %d\n", line, c);			
				break;
			}
			/* done handling unbalanced braces */
			
		}	
	}
	if (bracket > 0)
		printf("there are %d unbalanced brackets...\n", bracket);
}
