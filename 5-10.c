...
main(int argc, char **argv)
{
	...
	print("KR 5-10 reverse Polish calc from argv\n");
	while (--argc > 0) {
		strcat(arg, *++argv);
		if (argc == 1)
			break;
	...
	print("your input buffer: %s\n", arg);
	while ((type = getops2(s)) != 0) {
		switch (type) {
		case NUMBER:
			push(atof(s));
			break;
...
/* take a line as input instead of get/unget ch routines */
int
getops2(char s[])
{
	print("getop\n");
